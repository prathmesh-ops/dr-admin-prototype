...
export default function ModerationRequestsPage() {
  ...
  const getReasonBadge = (reason?: string | null) => {
    switch (reason?.toLowerCase()) {
      ...
      default:
        return <Badge className="bg-slate-50 text-slate-700 border-slate-200">Other</Badge>
    }
  }
    ...
    <div className="space-y-6 p-8 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 rounded-[50px]">
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight">Moderation Requests</h1>
            <p className="text-sm text-slate-600 mt-2">Review AI flagged posts and user reports</p>
          </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-3xl font-bold text-slate-900">{stats.aiFlags}</p>
              </div>
                    ...
                    <TableRow key={report.id} className="hover:bg-slate-50/50">
                      ...
                      <TableCell>
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{authorId}</p>
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          ...
                          {reasonDescriptions.length > 0 && (
                            <ul className="text-xs text-slate-600 space-y-0.5 max-w-xs">
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          <p className="text-sm text-slate-700">{reporterName}</p>
                          {caption && (
                            <p className="text-xs text-slate-500 line-clamp-2 max-w-xs">“{caption}”</p>
                          )}
                          {report.reporter_note && (
                            <p className="text-xs text-slate-500 max-w-xs">{report.reporter_note}</p>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        {reportedAt ? (
                          <>
                            <p className="text-sm text-slate-700">{new Date(reportedAt).toLocaleDateString()}</p>
                            <p className="text-xs text-slate-500">{new Date(reportedAt).toLocaleTimeString()}</p>
                          </>
                        ) : (
          ...
          {focusedPost ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between rounded-md border bg-slate-50 px-3 py-2 text-sm">
                <div>
                  <p className="font-semibold text-slate-900">{focusedPost.author_name}</p>
                  <p className="text-xs text-slate-500">{focusedPost.author_user_id}</p>
                </div>
              ...
              {focusedPost.flag_reasons?.length ? (
                <div className="space-y-2">
                  <p className="text-xs uppercase tracking-wide text-slate-500">Flag Reasons</p>
                  <div className="space-y-2 rounded-md border bg-white p-3">
                    {focusedPost.flag_reasons.map((reason, idx) => (
                      <div key={`${reason}-${idx}`} className="text-sm text-slate-700">
                        {reason}
                      </div>
                    ))}
                  </div>
            ...
            <div className="text-sm text-slate-500">Select a post to view its details.</div>
          )}
        </SheetContent>
      </Sheet>
    </div>
