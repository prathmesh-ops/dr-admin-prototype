...
export default function SalesDashboardPage() {
  ...
  const stats = [
    ...
    {
      title: "Conversions",
      value: formatNumber(conversionsSnapshot?.totalConverted ?? (kpiLoading ? 0 : conversions)),
      change: conversionsSnapshot
        ? `${formatNumber(conversionsSnapshot.convertedThisMonth)} this month`
        : kpiLoading ? "Loading..." : "No data",
      up: conversionsSnapshot ? conversionsSnapshot.conversionRate >= 30 : false,
      icon: TrendingUp,
          ...
          <CardContent className="p-6">
            <div className="grid gap-4 sm:grid-cols-2">
              {[
                {
                  label: "Calls Today",
                  value: activitiesSnapshot?.callLogsToday ?? (kpiLoading ? 0 : 0),
                },
                {
                  label: "Calls (7d)",
                ...
                //   value: funnelHealthSnapshot?.stuckLeads ?? (kpiLoading ? 0 : 0),
                // },
              ].map((metric) => (
                <div key={metric.label} className="rounded-xl bg-gradient-to-br from-slate-50/60 to-blue-50/30 px-4 py-5">
                  ...
                  <p className="text-3xl font-bold text-slate-900 tracking-tight tabular-nums mt-2">{formatNumber(metric.value)}</p>
                </div>
              ))}
            </div>
