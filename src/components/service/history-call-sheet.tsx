            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="doctor_id" className="text-sm font-semibold text-slate-900">
                    Select Doctor <span className="text-red-500">*</span>
                  </Label>
                  <Select value={form.doctor_id} onValueChange={(value) => onFormChange("doctor_id", value)}>
                    ...
                    <SelectContent>
                      {doctors.map((doctor, index) => (
                        <SelectItem key={doctor.staff_id} value={doctor.staff_id}>
                          Dr. {doctor.first_name || "John"} {doctor.last_name || `Doe ${index + 1}`}
                ...
                <div className="space-y-2">
                  <Label htmlFor="nutritionist_id" className="text-sm font-semibold text-slate-900">
                    Select Dietitian <span className="text-red-500">*</span>
                  </Label>
                  <Select value={form.nutritionist_id} onValueChange={(value) => onFormChange("nutritionist_id", value)}>
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder="Choose a deititian" />
                    </SelectTrigger>
                    <SelectContent>
                      {nutritionists.map((nutritionist, index) => (
                        <SelectItem key={nutritionist.staff_id} value={nutritionist.staff_id}>
                          {nutritionist.first_name || "Alex"} {nutritionist.last_name || `Lee ${index + 1}`}
                          {nutritionist.description && ` - ${nutritionist.description}`}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-2">
                  <Label htmlFor="fitness_coach_id" className="text-sm font-semibold text-slate-900">
                    Select Fitness Coach <span className="text-red-500">*</span>
                  </Label>
                  <Select
                    value={form.fitness_coach_id || "none"}
                    onValueChange={(value) => onFormChange("fitness_coach_id", value === "none" ? "" : value)}
                  >
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder="Choose a fitness coach (optional)" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">None</SelectItem>
                      {fitnessCoaches.map((coach, index) => (
                        <SelectItem key={coach.staff_id} value={coach.staff_id}>
                          {coach.first_name || "Taylor"} {coach.last_name || `Reed ${index + 1}`}
